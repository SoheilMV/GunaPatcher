...
namespace GunaPatcher
{
	class Program
	{
		static void Main(string[] args)
		{
			...
			try
			{
				...
				else
				{
					...
					foreach (MethodDef method in FindMethods(module))
                    {
		...
		private static IEnumerable<MethodDef> FindMethods(ModuleDef module)
		{
		...
		private static int CheckMethodsTypeSigCount(IList<MethodDef> methods, TypeSig typeSig)
        {
			int count = 0;
		...
		private static int CheckFieldsTypeSigCount(IList<FieldDef> fields, TypeSig typeSig)
		{
			int count = 0;
		...
		private static int CheckFieldsTypeSigCount(IList<FieldDef> fields, TypeDef typeDef)
		{
			int count = 0;
			foreach (var field in fields)
